...
export default function Home() {
  ...
  const [filter, setFilter] = useState<'All' | 'Characters' | 'Props' | 'Environments' | 'Doodles' | 'Speedsculpts'>('All');
  ...
  const categories: ('All' | 'Characters' | 'Props' | 'Environments' | 'Doodles' | 'Speedsculpts')[] = ['All', 'Characters', 'Props', 'Environments', 'Doodles', 'Speedsculpts'];
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-12 gap-6">
        <div className="flex items-center gap-2 bg-white/5 p-1 rounded-full border border-white/10 overflow-x-auto max-w-full no-scrollbar">
          {categories.map((cat) => (
            ...
            >
              {cat === 'All' ? 'Alle' : cat === 'Speedsculpts' ? '30 Min' : cat}
            </button>
          ))}
        </div>
